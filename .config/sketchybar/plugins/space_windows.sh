...
if [ "$SENDER" = "aerospace_workspace_change" ]; then
  ...
  sketchybar --animate tanh 10 --set space.$AEROSPACE_FOCUSED_WORKSPACE icon.highlight=true \
                         label.highlight=true \
                         background.border_color=$GREY

  # prev workspace space border color
  sketchybar --animate tanh 10 --set space.$AEROSPACE_PREV_WORKSPACE icon.highlight=false \
